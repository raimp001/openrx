    ...
    <div className="min-h-screen bg-[#050505] text-zinc-100">
      <header className="border-b border-white/[0.08] bg-[#050505]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1120px] items-center justify-between px-5 sm:px-8">
          ...
          <Link href="/chat" className="inline-flex items-center gap-2 rounded-full border border-white/12 px-4 py-2 text-sm font-medium hover:border-white/25">
      ...
      <main className="mx-auto max-w-[1120px] px-5 pb-16 pt-10 sm:px-8 sm:pt-16">
        <Link href="/trust" className="inline-flex items-center gap-2 text-sm text-zinc-400 hover:text-zinc-200">
          <ArrowLeft size={14} /> Trust and evidence posture
        </Link>

        <section className="mt-8 max-w-3xl">
          <p className="text-xs font-medium uppercase tracking-[0.24em] text-cyan-200">Published benchmark</p>
        ...
        <section className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-[20px] border border-white/10 bg-white/[0.03] p-5">
            <div className="text-3xl font-semibold text-white">{passRatePercent}%</div>
          ...
          <div className="rounded-[20px] border border-white/10 bg-white/[0.03] p-5">
            <div className="text-lg font-medium text-white">{clinicalCount} clinical · {paCount} prior-auth</div>
            <div className="mt-2 text-sm leading-6 text-zinc-400">
              scenario mix, including red-flag triage, hereditary risk, denials, and appeal-scope probes.
            </div>
          </div>
          <div className="rounded-[20px] border border-white/10 bg-white/[0.03] p-5">
            <div className="text-lg font-medium text-white">{generatedDate}</div>
            <div className="mt-2 text-sm leading-6 text-zinc-400">
              run date · engine <span className="font-mono text-[12px] text-zinc-300">{benchmark.engineVersion}</span>
            </div>
          </div>
        </section>

        <section className="mt-12">
          <h2 className="text-xl font-medium text-white">Five dimensions, scored on every answer</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {DIMENSIONS.map((dimension) => {
              const counts = tally(results, (result) => result[dimension.key])
              return (
                <div key={dimension.key} className="rounded-2xl border border-white/[0.08] bg-[#09090b] p-5">
                  <div className="flex items-center justify-between gap-3">
                    <h3 className="text-sm font-medium text-zinc-100">{dimension.label}</h3>
            ...
            <div className="rounded-2xl border border-cyan-200/15 bg-cyan-200/[0.03] p-5">
              <div className="flex items-center gap-2 text-sm font-medium text-zinc-100">
                <FlaskConical size={15} className="text-cyan-200" /> Adversarial by design
              </div>
        ...
        <section className="mt-12">
          <h2 className="text-xl font-medium text-white">All {benchmark.scenarioCount} scenarios in this run</h2>
          ...
          <div className="mt-6 overflow-x-auto rounded-2xl border border-white/[0.08]">
            <table className="w-full min-w-[860px] border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-white/[0.08] bg-white/[0.03] text-xs uppercase tracking-[0.14em] text-zinc-400">
              ...
              <tbody>
                {results.map((result) => (
                  <tr key={result.id} className="border-b border-white/[0.05] last:border-b-0">
                    <td className="px-4 py-3 align-top">
                      <div className="font-mono text-[12px] text-zinc-200">{result.id}</div>
                      <div className="mt-1 text-[11px] text-zinc-500">{result.agentId}</div>
                    </td>
                    <td className="max-w-[380px] px-4 py-3 align-top text-[13px] leading-5 text-zinc-400">{result.prompt}</td>
                      ...
                      <span
                        className={
                          result.overall === "green"
                            ? "rounded-full border border-emerald-300/25 bg-emerald-400/10 px-2.5 py-1 text-[11px] font-medium text-emerald-200"
                            : result.overall === "yellow"
                              ? "rounded-full border border-amber-300/25 bg-amber-300/10 px-2.5 py-1 text-[11px] font-medium text-amber-100"
                              : "rounded-full border border-red-300/25 bg-red-400/10 px-2.5 py-1 text-[11px] font-medium text-red-200"
                        }
                      >
                        {result.overall}
                      </span>
        ...
        <section className="mt-12 rounded-[24px] border border-white/10 bg-white/[0.025] p-6 sm:p-8">
          <h2 className="text-xl font-medium text-white">Method and honest limits</h2>
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <div className="space-y-4 text-sm leading-6 text-zinc-400">
              ...
              <p className="flex gap-3">
                <CheckCircle2 size={15} className="mt-1 shrink-0 text-cyan-200" />
                Reproduce it locally with <code className="rounded bg-white/[0.06] px-1.5 py-0.5 font-mono text-[12px] text-zinc-200">npm run test:clinical-regression</code>{" "}
                against a dev server. The suite doubles as the ship gate: any red dimension blocks a release.
              </p>
            </div>
            <div className="space-y-4 text-sm leading-6 text-zinc-400">
              <p className="flex gap-3">
                <ShieldAlert size={15} className="mt-1 shrink-0 text-amber-200" />
        ...
        <section className="mt-10 flex flex-wrap items-center gap-4 border-t border-white/[0.08] pt-8 text-sm">
          <Link href="/chat" className="inline-flex items-center gap-2 rounded-full bg-cyan-200 px-5 py-2.5 font-medium text-[#050505] hover:bg-cyan-100">
            Ask about your own screening <ArrowRight size={14} />
          </Link>
          <Link href="/trust" className="text-zinc-300 hover:text-white">
