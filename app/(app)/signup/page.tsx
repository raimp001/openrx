    ...
    <div className="animate-slide-up mx-auto max-w-2xl space-y-6">
      ...
      <section className="surface-card space-y-5 p-6 sm:p-8">
        <ol className="space-y-3">
          {steps.map((step, index) => (
            <li key={step} className="flex items-start gap-3 rounded-[18px] border border-white/10 bg-white/[0.04] px-4 py-3.5">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-cyan-700 text-[11px] font-bold text-white">
                {index + 1}
              </span>
        ...
        <div className="flex flex-wrap items-center gap-4 pt-1">
          ...
          <Link href="/login" className="text-[13px] font-semibold text-cyan-700 underline-offset-4 hover:underline">
      ...
      <p className="text-xs leading-5 text-muted">
        Prefer to look around first? <Link href="/screening" className="text-cyan-700 underline-offset-4 hover:underline">Try the screening navigator</Link> or{" "}
        <Link href="/chat" className="text-cyan-700 underline-offset-4 hover:underline">ask a question in chat</Link> — no account required.
      </p>
    </div>
  )
}
