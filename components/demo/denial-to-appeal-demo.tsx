  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
      <div className="flex flex-wrap items-center gap-2 text-xs">
        <span className="rounded-full border border-cyan-300/20 bg-cyan-300/[0.07] px-2 py-1 font-medium text-cyan-800">
          {source.organization}
        </span>
        <span className="text-zinc-500">{source.version}</span>
      ...
      <p className="mt-2 text-sm leading-6 text-zinc-500">{source.relevance}</p>
    </div>
  )
}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm font-medium leading-6 text-zinc-900">{scenario.title}</p>
        </div>
        <ChevronRight size={16} className={cn("mt-1 shrink-0 text-zinc-600 transition", selected && "text-cyan-700")} />
      </div>
    </button>
  )
}
    ...
    <div className="min-h-screen bg-white text-zinc-900">
      <header className="border-b border-zinc-200 bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1240px] items-center justify-between px-5 sm:px-8">
          ...
          <nav className="flex items-center gap-4 text-sm text-zinc-500" aria-label="Demo navigation">
            <Link href="/trust" className="hidden hover:text-zinc-900 sm:block">Trust</Link>
            <Link href="/chat" className="rounded-full border border-zinc-200 px-4 py-2 font-medium text-zinc-800 transition hover:border-zinc-300">
              Open clinical chat
            </Link>
          </nav>
        </div>
      ...
      <main className="mx-auto max-w-[1240px] px-5 pb-14 pt-10 sm:px-8 sm:pt-16">
        <section className="grid gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:items-start">
          <div className="lg:sticky lg:top-10">
            <p className="text-xs font-medium uppercase tracking-[0.24em] text-cyan-700">Prior authorization sandbox</p>
            <h1 className="orx-display-heading mt-5 text-[clamp(2.35rem,4.5vw,3.7rem)] text-zinc-900">
            ...
            <div className="mt-7 flex flex-wrap gap-2 text-xs text-zinc-600">
              {["Synthetic cases only", "Clinician review required", "No live payer transmission"].map((item) => (
                <span key={item} className="rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1.5">
                  {item}
                </span>
              ))}
            </div>
            ...
            <Link href="/trust" className="mt-6 inline-flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-700">
          ...
          <section
            ...
            className="overflow-hidden rounded-[28px] border border-zinc-200 bg-white shadow-[0_24px_90px_rgba(15,23,42,0.10)]"
          >
            <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-4 sm:px-7">
              <div>
                <p className="text-sm font-medium text-zinc-800">Denial-to-appeal workspace</p>
            ...
            <div className="grid grid-cols-4 border-b border-zinc-200">
              {STEPS.map((step, index) => (
                <div key={step.label} className="border-r border-zinc-100 px-3 py-3 last:border-r-0 sm:px-5">
                  <p className="text-[11px] font-semibold text-cyan-700">{String(index + 1).padStart(2, "0")} {step.label}</p>
            ...
            {!scenario ? (
              <div className="flex min-h-[570px] flex-col items-center justify-center px-8 text-center">
                ...
                <h2 className="mt-5 text-xl font-medium text-zinc-900">Pick a denial scenario</h2>
              ...
              <div className="space-y-6 p-5 sm:p-7">
                <div className="ml-auto max-w-[92%] rounded-2xl rounded-br-md bg-zinc-100 px-5 py-4">
                  <p className="text-xs font-medium uppercase tracking-[0.15em] text-zinc-500">Denial received</p>
                  <p className="mt-3 text-sm leading-6 text-zinc-800">{scenario.denialReason}</p>
                  <p className="mt-3 text-sm font-medium text-zinc-900">{scenario.request}</p>
                </div>

                <div className="max-w-[96%] rounded-2xl rounded-bl-md border border-zinc-200 bg-white p-5">
                  <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.15em] text-cyan-700">
                  ...
                  {!evidence ? (
                    <button
                      ...
                      className="mt-5 inline-flex items-center gap-2 rounded-xl bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-zinc-700 disabled:cursor-wait disabled:opacity-60"
                    >
                      <FileText size={15} />
                ...
                {error ? (
                  <div data-testid="demo-error" className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                    {error}
                  </div>
                ) : null}
                ...
                {evidence ? (
                  <div data-testid="demo-evidence" className="max-w-[96%] rounded-2xl rounded-bl-md border border-cyan-300/15 bg-cyan-300/[0.025] p-5">
                    <div className="flex items-center gap-2 text-sm font-medium text-zinc-900">
                      <ShieldCheck size={16} className="text-cyan-700" /> Evidence retrieved
                    </div>
                    <p className="mt-3 text-sm leading-6 text-zinc-500">{evidence.boundary}</p>
                    ...
                    {!appeal ? (
                      <button
                        ...
                        className="mt-5 inline-flex items-center gap-2 rounded-xl bg-cyan-700 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-cyan-800 disabled:cursor-wait disabled:opacity-60"
                      >
                        <FileText size={15} />
                ...
                {appeal ? (
                  <div data-testid="demo-appeal" className="max-w-[96%] rounded-2xl rounded-bl-md border border-zinc-200 bg-white p-5">
                    <p className="text-xs font-medium uppercase tracking-[0.15em] text-cyan-700">Appeal draft</p>
                    <h2 className="mt-3 text-base font-medium leading-6 text-zinc-900">{appeal.subject}</h2>
                    ...
                    <div className="mt-5 rounded-xl border border-zinc-200 bg-zinc-50 p-4">
                      ...
                      <ul className="mt-3 grid gap-2 text-sm text-zinc-600 sm:grid-cols-2">
                        {appeal.documentChecklist.map((item) => (
                          <li key={item} className="flex items-start gap-2"><CheckCircle2 size={14} className="mt-1 shrink-0 text-cyan-700" />{item}</li>
                        ))}
                      </ul>
                    </div>
                    <p className="mt-4 text-xs leading-5 text-amber-700">{appeal.reviewRequired}</p>
                    {!submission ? (
                      <button
                        type="button"
                ...
                {submission ? (
                  <div data-testid="demo-submission-success" className="max-w-[96%] rounded-2xl rounded-bl-md border border-emerald-200 bg-emerald-50 p-5">
                    <div className="flex items-center gap-2 text-sm font-medium text-emerald-700">
                      <CheckCircle2 size={17} /> Sandbox handoff accepted
                    </div>
                    <p className="mt-3 text-sm leading-6 text-zinc-700">{submission.notice}</p>
                    <div className="mt-4 flex flex-wrap gap-4 text-xs">
                      <div>
                        <p className="text-zinc-500">Tracking number</p>
                        <p className="mt-1 font-mono text-zinc-800">{submission.trackingNumber}</p>
                      </div>
                      <div>
                        <p className="text-zinc-500">Transmission</p>
                        <p className="mt-1 text-emerald-700">Simulated only</p>
                      </div>
                    </div>
                    <pre className="mt-5 overflow-x-auto rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-xs leading-6 text-zinc-600">
        ...
        <section className="mt-16 grid gap-4 border-t border-zinc-200 pt-7 text-sm text-zinc-500 sm:grid-cols-3">
          <div className="flex gap-3">
            <LockKeyhole size={17} className="shrink-0 text-zinc-600" />
            <p>No PHI, account, or wallet is required in the sandbox.</p>
          </div>
          <div className="flex gap-3">
            <ShieldCheck size={17} className="shrink-0 text-zinc-600" />
            <p>Sources are linked and licensed-content boundaries are visible.</p>
          </div>
          <div className="flex gap-3">
            <FileText size={17} className="shrink-0 text-zinc-600" />
